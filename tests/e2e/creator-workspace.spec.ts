...
test('creator identity opens a bound native conversation and the same editable artifact', async ({ page, classroom }, info) => {
  ...
  await page.getByTestId('agent-role').click(); await page.getByRole('dialog', { name: '智能体身份', exact: true }).getByRole('button', { name: '创作者', exact: true }).click();
  ...
  await expect(page.getByTestId('agent-role')).toHaveAttribute('value', 'creator');
  ...
  await page.getByTestId('agent-role').click(); await page.getByRole('dialog', { name: '智能体身份', exact: true }).getByRole('button', { name: '教学者', exact: true }).click();
