...
test('a stale directory proposal rechecks the current tree and waits for a new confirmation', async ({ page, dsh }, testInfo) => {
  ...
  const sections = ['直接求值', '配角与凑角（结构变形、系数调整）', '和差公式与弦切互化', '结构证明与给值求角'];
  await sendInput(page, '[tools]' + JSON.stringify([{ name: 'read_skeleton', arguments: { materialId: book.materialId } }, { name: 'propose_skeleton', arguments: { materialId: book.materialId, change: { nodes: sections.map(title => node('和差公式/' + title)) } } }]));
  await expect.poll(async () => value(await client.rpc<ProposalView[]>('studyforgeProposals/list', { input: {} })).length).toBe(1);
  const inline = page.getByTestId('inline-proposal');
  const directory = inline.getByTestId('proposal-directory');
  for (const title of sections) await expect(directory.getByText(title, { exact: true })).toBeVisible();
  await expect(directory.locator('.sf-directory-tree .sf-directory-tree > li')).toHaveCount(4);
  await expect(inline.getByTestId('proposal-title')).toHaveCount(0);
  await expect(inline).not.toContainText('细节暂时改不了');
  await expect(inline).not.toContainText('还没有保存的项');
  await expect(inline).not.toContainText('版提案');
  await inline.getByTestId('proposal-edit').click();
  await expect(inline.getByTestId('draft-skeleton-path')).toHaveCount(4);
  await inline.getByTestId('draft-cancel').click();
  await page.screenshot({ path: testInfo.outputPath('directory-inline.png'), fullPage: true });
  ...
  expect(saved.nodes.map(node => node.path)).toEqual(['和差公式', '二倍角', ...sections.map(title => '和差公式/' + title)]);
});
