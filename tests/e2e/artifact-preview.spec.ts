...
test('HTML artifact runs in sandbox and installs a versioned classroom skill', async ({ page, classroom }, info) => {
  await enterClassroom(page, classroom.authUrl); await page.getByTestId('agent-role').selectOption('creator');
  ...
  await page.getByRole('button', { name: '回到创作对话', exact: true }).click(); await page.getByTestId('agent-role').selectOption('teacher');
