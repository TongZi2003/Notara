import { test, expect, enterClassroom, sendInput, openLessonSettings, openLessonMaterials } from './fixtures/classroom.ts';
...
test('the lesson right column is one material map that opens originals and cards in place', async ({ page, classroom }, info) => {
  ...
  // The materials workbench is one workspace pane: the lesson's map, nothing
  // else — never a raw document preview — and the workspace nav is the way in.
  ...
  // The workbench now defaults to the whole-library whiteboard; this test is
  // about the lesson's own map, so scope it back to what this lesson uses.
  await panel.getByTestId('workbench-scope').selectOption('lesson');
  await expect(panel).toBeVisible();
  ...
  // Flush against the top: the workspace pane frame's own border and padding
  // land inside this gap, so the bound guards a phantom header, not a pixel.
  expect(scopeBox.y - panelBox.y).toBeLessThanOrEqual(24);
  expect(mapBox.y - scopeBox.y - scopeBox.height).toBeLessThanOrEqual(24);
  ...
  // The workspace pane can be closed and reopened, keeping open pages and hierarchy.
  ...
  await page.getByTestId('workspace-open-materials').click();
  await expect(page.getByTestId('lesson-materials-pane')).toHaveCount(3);
  // The materials pane can take the whole workspace surface.
  await page.getByRole('button', { name: '资料工作台布局', exact: true }).click();
  await page.getByRole('button', { name: '只看资料工作台', exact: true }).click();
  await expect(page.getByTestId('lesson-materials-pane')).toHaveCount(3);
  ...
  await page.getByRole('button', { name: '对话与资料', exact: true }).click();

  // Reading opened no original session and called no model.
  expect(await requests()).toBe(quiet);

  // 本课设置 lives in the workspace layout menu; it still owns usage and the
  // learning profile.
