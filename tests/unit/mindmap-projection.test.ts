...
import { bookMindNodes, lessonMindProjection } from '../../packages/client/src/materials/lesson-materials-mindmap.ts';
...
test('a book structure becomes nodes that keep their own parents, kinds and positions', () => {
  ...
  expect(nodes[0]).toMatchObject({ kind: 'book', hint: '书', parent: undefined });
  expect(nodes[1]).toMatchObject({ kind: 'section', hint: '原文', parent: 'book' });
...
test('a lesson row is a book node only when the file really is a book, and cards stay their own nodes', () => {
  ...
  expect(section).toMatchObject({ kind: 'section', hint: '原文', parent: 'row:material:material:m1@v1' });
