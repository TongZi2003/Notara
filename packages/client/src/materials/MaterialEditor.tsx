...
export function MaterialEditor({ ctx, view, version, onSaved }: { ctx: Context; view: MaterialView; version: MaterialVersion; onSaved(saved: MaterialView): void }): React.JSX.Element | null {
  ...
  return <section className="sf-material-coedit" data-testid="classroom-markdown-editor"><div className="sf-org-actions"><button className="sf-quiet" disabled={busy} onClick={() => { void load().then(text => { setBody(text); setBase(view.revision); }).catch(() => setNotice('原文暂时读不出来。')); }}>编辑 Markdown</button></div>
    {body !== undefined && <><textarea aria-label="编辑原文正文" data-testid="classroom-markdown-draft" value={body} onChange={e => setBody(e.target.value)} /><div className="sf-org-actions"><button className="sf-action" data-testid="material-save-version" disabled={busy} onClick={() => {
