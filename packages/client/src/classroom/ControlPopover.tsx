...
export function ControlPopover({ label, title, className = '', menuClassName = '', testId, triggerTestId, value, disabled, chevron = true, children }: {
  ...
  useLayoutEffect(() => {
    ...
    const place = (): void => {
      ...
      setPosition({ left: Math.max(margin, Math.min(anchor.left, window.innerWidth - width - margin)), top: upwards ? Math.max(margin, anchor.top - Math.min(box.scrollHeight, height) - 7) : anchor.bottom + 7, maxHeight: Math.max(100, height) });
    };
