...
import { ArtifactManifestSchema, type ArtifactView, type CreationRecord } from '@studyforge/contracts/creation';
...
export function initializeProject(host: Context, record: CreationRecord, content?: string): void {
  ...
  if (!existsSync(path)) writeFileSync(path, content ?? (record.initial.kind === 'html' ? '<!doctype html><html lang="zh"><meta charset="utf-8"><title>教学演示</title><body><main>从一个想演示的问题开始。</main></body></html>' : '# ' + record.initial.title + '\n\n'), { flag: 'wx', mode: 0o600 });
}
export function readProject(host: Context, record: Saved<CreationRecord>): ArtifactView {
  ...
  const files = readdirSync(root).filter(path => ['manifest.json', 'content.md', 'index.html'].includes(path)).map(path => {
...
export function saveProjectFile(host: Context, context: HostContext, ref: string, path: string, expectedDigest: string, content: string): ArtifactView {
  ...
  if (!['manifest.json', 'content.md', 'index.html'].includes(path) || canonicalPath(file, root) !== file) throw new Error('creation_file_invalid');
