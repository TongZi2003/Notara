...
export function registerCardTools(host: Context): void {
  ...
  host.effect(() => host.tools.register({ name: 'read_cards', description: '一次读取1至20张普通卡的完整正文、来源、笔记和学习记录，按targets顺序返回。引用来自list_cards或search_learning；任何目标读取失败则整次报错，不把缺失说成空卡。Host从成功结果绑定每张卡的真实版本，后续update_card/propose_review可直接使用；读取不表示学过。',
    ...
    output: { schema: toolSchema(CardBatchReadResultSchema), render: (_args, value) => [{ type: 'text', text: JSON.stringify(CardBatchReadResultSchema.parse(value)) }, ...entityReferenceContent(value)] },
    async execute(args, execution) {
  ...
  const output = { schema: toolSchema(CardViewSchema), render: (_args: unknown, value: unknown) => [{ type: 'text' as const, text: JSON.stringify(CardViewSchema.parse(value)) }, ...entityReferenceContent(value)] };
  ...
  host.effect(() => host.tools.register({ name: 'register_cards', description: '诊断收尾或获准的独立命题产物登记：一批先全验，再由Host原子保存为未学普通卡。通常课堂新内容用propose_card让学生确认；保存不产生复习。', parameters: toolSchema(batchInput),
    output: { schema: toolSchema(batchOutput), render: (_args, value) => [{ type: 'text', text: JSON.stringify(batchOutput.parse(value)) }, ...entityReferenceContent(value)] },
    async execute(args, execution) {
