...
import { forestLayout } from './forest-client.js';
...
export function visibleTagOptions(tags, query = '', selected = [], limit = 36) {
  ...
  const matches = values.filter(tag => !needle || tag.toLocaleLowerCase().includes(needle));
  const items = [...values.filter(tag => chosen.has(tag)), ...matches.filter(tag => !chosen.has(tag))].slice(0, Math.max(1, limit));
  const matched = new Set(matches);
  return { items, hidden: Math.max(0, matches.length - items.filter(tag => matched.has(tag)).length) };
}
...
/** What a node is on the star map, from the graph alone. The Host projection
 * only lends it light when it agrees, so a stale read is shown as unknown
 * instead of borrowing another shape's brightness. */
export const starKindOf = node => node?.childCount > 0 ? 'parent' : node?.kind === 'page' && node?.type === 'card' ? 'leaf' : 'unlinked';
export function starOf(node, stars) {
  const kind = starKindOf(node), entry = stars?.nodes?.[node?.path];
  return { key: node.path, title: node.title, kind, light: entry?.kind === kind ? entry : undefined, node };
}
...
.nv-tag-filter{flex:1 1 100%;min-width:220px}
.nv-tag-filter>summary{display:inline-flex;align-items:center;cursor:pointer;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:28px}
.nv-tag-filter>input{display:block;width:min(280px,100%);height:30px;margin:6px 0;border:1px solid var(--dsw-alias-border-l1);border-radius:var(--nv-r-md,10px);padding:0 9px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:inherit}
.nv-tag-filter-list{display:flex;flex-wrap:wrap;align-items:flex-start;gap:6px;height:54px;max-height:min(50vh,360px);overflow:auto;resize:vertical;padding:2px 0 4px}
.nv-tag-filter-list.nv-tag-filter-expanded{height:240px;overflow:auto}
.nv-tag-filter-list>button{border:0;border-radius:999px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);padding:4px 9px;cursor:pointer;font:12px var(--dsw-font-family,system-ui)}
.nv-tag-filter-list>button[aria-pressed=true]{background:var(--dsw-alias-interactive-bg-active);color:var(--dsw-alias-label-primary)}
.nv-tag-filter-more{color:var(--dsw-alias-label-caption);font-size:11px;line-height:24px}
.nv-tag-filter-toggle{border:0;background:transparent;color:var(--dsw-alias-label-tertiary);padding:2px 0;cursor:pointer;font:11px var(--dsw-font-family,system-ui)}
.nv-tag-filter-toggle:hover{color:var(--dsw-alias-label-primary)}
.nv-card-math .katex{font-size:1.02em}.nv-card-math-display{display:block;overflow:auto}
.nv-graph-tag-filter .nv-tag-filter{display:flex;flex-wrap:wrap;gap:6px;align-items:center}
.nv-graph-tag-filter .nv-tag-filter>input{margin:0}
.nv-graph-tag-filter .nv-tag-filter-list{flex-basis:100%;height:54px;max-height:min(50vh,360px)}
.nv-graph-tag-filter .nv-tag-filter-list.nv-tag-filter-expanded{height:240px;max-height:min(60vh,520px)}
...
export function createVaultViews(React, { STYLE, IconButton, Menu, Dialog }) {
  ...
  /** The Host's read-only star projection. A failed refresh keeps the last
  ...
  function Frame({ title, status, tools, children, flush }) {
    return h('div', { className: 'nv-views', style: STYLE.page }, h('style', null, CSS), h('style', null, CANVAS_CSS), h('style', null, STAR_CSS),
      h('header', { className: flush ? 'nv-view-top nv-view-top-flush' : 'nv-view-top' }, h('strong', { style: STYLE.brand }, title), tools,
  ...
  function CardsView(props) {
    ...
    const [search, setSearch] = useState(''), [source, setSource] = useState(''), [grouped, setGrouped] = useState(false), [tagSearch, setTagSearch] = useState(''), [tagExpanded, setTagExpanded] = useState(false);
    ...
    const tags=[...new Set(cards.flatMap(node=>node.tags))].sort(), tagOptions=visibleTagOptions(tags,tagSearch,tag,tagExpanded?tags.length:36);
    ...
    const excerpt = text => {
      const plain = String(text).replace(/!?\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g, (_match, target, label) => label || target.split('/').at(-1).replace(/\.md$/, ''));
      return mathLabelParts(plain).map((part, i) => {
      if(part.kind==='math'){const math=renderMath(part.source,part.display);if(math)return h('span',{key:i,className:part.display?'nv-card-math nv-card-math-display':'nv-card-math',dangerouslySetInnerHTML:{__html:math}});}
      return part.value;
      });
    };
    return h(Frame, { title: '卡片', status: error || `${filtered.length} 项${insights ? ` · ${insights} 张锦囊` : ''}`, flush: true, tools: h(React.Fragment, null,
      ...
      h('div',{className:'nv-card-filters','aria-label':'卡片筛选'},
        ...
        h('details',{className:'nv-tag-filter'},h('summary',null,`标签${tags.length?` · ${tags.length}`:''}${tag?` · #${tag}`:''}`),
          h('input',{'aria-label':'搜索标签',placeholder:'搜索标签…',value:tagSearch,onChange:e=>setTagSearch(e.target.value)}),
          h('div',{className:`nv-tag-filter-list${tagExpanded?' nv-tag-filter-expanded':''}`},tagOptions.items.map(value=>h('button',{key:value,'aria-pressed':tag===value,onClick:()=>setTag(tag===value?'':value)},'#'+value)),tagOptions.hidden>0&&h('span',{className:'nv-tag-filter-more'},`还有 ${tagOptions.hidden} 个标签，请继续搜索`)),
          h('button',{type:'button',className:'nv-tag-filter-toggle','aria-expanded':tagExpanded,onClick:()=>setTagExpanded(value=>!value)},tagExpanded?'收起标签':'展开全部标签'))),
      ...
      h('div', { className: 'nv-cards-scroll' }, loading || (reviewFilter!=='all'&&!reviews&&!reviewError) ? h('div', { style: STYLE.empty }, '正在读取…') : !cards.length ? h('div', { style: STYLE.empty }, '还没有卡片。从文件视图选取一段内容开始。') : !filtered.length ? h('div', { style: STYLE.empty }, reviewError&&reviewFilter!=='all'?'复习状态读取失败，请稍后重试。':'没有符合条件的卡片。')
        : [...groups.entries()].sort(([a], [b]) => String(a).localeCompare(String(b),'zh-CN')).map(([level, nodes]) => h('section', { key: String(level) },
          ...
          h('div', { className: 'nv-card-grid' }, nodes.map(node => h('article', { className: 'nv-card', key: node.path },
            h('h3', null, h('button', { style: { ...STYLE.link, fontSize: 16, color: 'var(--dsw-alias-label-link, var(--dsw-alias-label-primary))', textDecoration: 'none' }, 'aria-label': `打开卡片 ${node.title}`, onClick: () => props.openView(VIEW_IDS.assets, node.path) }, node.title),
            ...
            h('div', { className: 'nv-card-source' },
              node.sources.map((item, index) => h('button', { key: `${item.path}:${index}`, className: 'nv-link', style: { ...STYLE.link, textAlign: 'left', fontSize: 12 }, 'aria-label': `阅读来源 ${sourceLabel(item)}`, onClick: () => props.openView(VIEW_IDS.assets, focusFor(item.path, item.locator)) }, '↩ '+sourceLabel({...item,path:graph.nodes.find(n=>n.path===item.path)?.title||item.path}))),
              node.parent && !node.sources.some(item => item.path === node.parent) && h('button', { className: 'nv-link', style: STYLE.link, onClick: () => props.openView(VIEW_IDS.graph, node.parent) }, `${parentLabelOf(graph, node.parent)}：${graph.nodes.find(n=>n.path===node.parent)?.title||node.parent}`),
              node.type==='card' && h('button', { className: 'nv-link', style: { ...STYLE.link, textAlign: 'left', fontSize: 12 }, onClick: () => props.openView(VIEW_IDS.calendar, node.path) }, '复习安排'),
              h('button', { className: 'nv-link', style: { ...STYLE.link, textAlign: 'left', fontSize: 12 }, onClick: () => props.openView(VIEW_IDS.graph, node.path) }, '在图谱中查看')))))))));
  }
  ...
  function GraphView(props) {
    ...
    // 结构 / 星图 read one projection with one selection, filter and focus; the
    // light theme reads the same light as a 森林.
    const [view,setView]=useRemembered(state,'view','structure'), forest=useSky()==='forest';
    ...
    const progressive=graph.nodes.length>180&&!showAll;
    const visible=useMemo(()=>progressive?collapseVaultGraph(filtered,expanded):filtered,[filtered,progressive,expanded]);
    const taggedCards=useMemo(()=>taggedCardNodes(graph,tags),[graph,tags]);
    ...
    const splitAllowed=!!node&&(!LIBRARY_TYPES.has(node.type)||children.length>0);
    const cardKind=node&&CARD_TYPE_LABEL[node.type]?CARD_TYPE_LABEL[node.type]:'';
    const select=path=>{setSelected(path);setMenu(null);setNotice('');if(progressive){const target=graph.nodes.find(item=>item.path===path);if(target?.childCount>0)setExpanded(previous=>previous.includes(path)?previous:[...previous,path]);}if(!detailsOpen){clearTimeout(detailTimer.current);detailTimer.current=setTimeout(()=>setDetailsOpen(true),250);}};
    ...
    const canvasNodes=useMemo(()=>visible.nodes.map(item=>({key:item.path,title:item.title,role:item.type==='insight'?'insight':item.role,hint:[roleLabelOf(item),CARD_TYPE_LABEL[item.type]].filter(Boolean).join(' · '),node:item})),[visible]);
    // The sky is laid out over the whole filtered slice, so opening a layer
    // lights stars where they already belong instead of moving the others.
    ...
    const starNodes=useMemo(()=>view==='stars'?visible.nodes.map(item=>({...starOf(item,stars),hint:[roleLabelOf(item),CARD_TYPE_LABEL[item.type]].filter(Boolean).join(' · ')})):[],[visible,stars,view]);
    const context=(item,event)=>{clearTimeout(detailTimer.current);const bounds=root.current.getBoundingClientRect();setMenu({path:item.key,x:Math.max(0,Math.min(event.clientX-bounds.left,bounds.width-230)),y:Math.max(0,Math.min(event.clientY-bounds.top,bounds.height-200))});};
    const bring=async target=>{
      ...
      try {
        ...
        const intentForTarget=LIBRARY_TYPES.has(target.type)&&!targetChildren.length?'':intent;
        if(!props.onBring(result.value,undefined,1,intentForTarget))setNotice('当前对话输入框正在变化，请稍后重试。');
      }catch{setNotice('无法读取这个文件，请刷新后重试。');}
    };
    const bringTaggedCards=async()=>{
      setMenu(null);
      if(!tags.length||!taggedCards.length){setNotice('当前标签没有可带入的卡片。');return;}
      try{
        const values=await Promise.all(taggedCards.map(async target=>{
          const result=await vault.read({path:target.path});
          if(!result?.ok)throw new Error('read');
          return result.value;
        }));
        const pins=values.map(value=>({kind:'page',sessionId:props.sessionId,path:value.path,revision:value.revision,title:value.title}));
        const intent=`请基于这 ${pins.length} 张带有 ${tags.map(tag=>'#'+tag).join('、')} 的卡片，比较它们的共同结构、差异与相互联系；先概括关系，再指出需要核对的地方。`;
        if(typeof props.onBringMany!=='function'||!props.onBringMany(pins,intent))setNotice('当前对话输入框正在变化，请稍后重试。');
        else setNotice(`已将 ${pins.length} 张卡片带入对话`);
      }catch{setNotice('无法读取筛选出的卡片，请刷新后重试。');}
    };
    const graphTagOptions=visibleTagOptions(groups.map(group=>group.tag),tagSearch,tags,tagExpanded?groups.length:36), visibleGroups=graphTagOptions.items.map(tag=>groups.find(group=>group.tag===tag)).filter(Boolean);
    const tagPanel = filtering && h('div', { className: 'nv-bar nv-graph-tag-filter', style: { flexWrap: 'wrap', padding: '8px 16px', gap: 8 }, 'aria-label': '图谱标签' },
      groups.length
        ? h('div',{className:'nv-tag-filter'},
          h('input',{'aria-label':'搜索图谱标签',placeholder:'搜索标签…',value:tagSearch,onChange:e=>setTagSearch(e.target.value)}),
          h('div',{className:`nv-tag-filter-list${tagExpanded?' nv-tag-filter-expanded':''}`},visibleGroups.map(group => h('div', { key: group.tag, className: 'nv-tag-group' },
            ...
            openGroup === group.tag && h('div', { className: 'nv-tag-members' },
              ...
              group.paths.slice(0,40).map(path => h('button', { key: path, className: 'nv-link', style: { ...STYLE.link, fontSize: 12 }, 'aria-label': `聚焦 ${path}`, onClick: () => select(path) }, titles.get(path) ?? path)),
              group.paths.length>40&&h('span',{className:'nv-tag-filter-more'},`还有 ${group.paths.length-40} 个文件`)))),
          graphTagOptions.hidden>0&&h('span',{className:'nv-tag-filter-more'},`还有 ${graphTagOptions.hidden} 个标签，请继续搜索`)),
          h('button',{type:'button',className:'nv-tag-filter-toggle','aria-expanded':tagExpanded,onClick:()=>setTagExpanded(value=>!value)},tagExpanded?'收起标签':'展开全部标签'))
        : h('span', { style: STYLE.notice }, '文件还没有标签。'),
      tags.length > 0 && btn('清除筛选', () => setTags([])));
    const tools=h(React.Fragment,null,
      h('div',{className:'nv-route-views',role:'group','aria-label':'图谱视图'},[['structure','结构'],['stars',forest?'森林':'星图']].map(([id,label])=>h('button',{key:id,type:'button','aria-pressed':view===id,onClick:()=>setView(id)},label))),
      ...
      tags.length>0&&h(IconButton,{icon:'chat',label:`带入当前筛选的 ${taggedCards.length} 张卡片`,disabled:!taggedCards.length,onClick:()=>{void bringTaggedCards();}}),
      tags.length>0&&h('span',{style:STYLE.notice},tags.map(tag=>'#'+tag).join(' ')),
      progressive&&btn('展开全部',()=>setShowAll(true)),
      showAll&&graph.nodes.length>180&&btn('按层级展开',()=>setShowAll(false)));
        ...
        node&&detailsOpen&&h(React.Fragment,null,
          ...
          h('aside',{className:'nv-graph-pane','aria-label':'节点详情'},
            ...
            h('div',{style:{display:'flex',gap:4,marginBottom:20}},
              ...
              h(IconButton,{icon:'chat',label:splitAllowed ? '带入对话拆分' : '带入对话',onClick:()=>bring(node)})),
            ...
            node.parent&&h('p',{style:STYLE.notice},`${parentLabelOf(graph,node.parent)}：`,h('button',{className:'nv-link',style:STYLE.link,onClick:()=>select(node.parent)},titles.get(node.parent)??node.parent)),
            ...
            node.sources.length>0&&h('details',{style:{fontSize:12,marginTop:24}},h('summary',{style:{cursor:'pointer',color:'var(--dsw-alias-label-secondary)'}},'来源'),
              node.sources.map((source,index)=>h('p',{key:index},h('button',{className:'nv-link',style:STYLE.link,'aria-label':'跳回原文 '+sourceLabel(source),onClick:()=>props.openView(VIEW_IDS.assets,focusFor(source.path,source.locator))},sourceLabel(source))))))),
