...
export const VAULT_TOOL_CONTRACTS=[
  {name:'write_lesson_board',description:'在当前课堂白板指定区域写入学生可见的Markdown板书。同名title替换该区域全文，新的title创建区域；补写时保留仍有效的原文。Host绑定文件和块身份。先给title，再给body，前端可在该处流式显示文字。引用真实使用的资料用[[已读的Vault相对路径|资料名]]。只写已经向学生呈现的内容，不写隐藏答案、教师分析或原始私密对话。可选的interactive只允许登记过的math/parabola结构化场景，不能传HTML、脚本、路径或ID。保存失败后重新读取确认再改，不宣称已保存。',parameters:object({title:{...string('新区域的简短标题，或当前已存在的准确标题。'),maxLength:160},body:{...string('该区域完整Markdown正文。局部高亮支持<mark data-color="blue|green|orange|pink">文字</mark>，不是整块背景。'),maxLength:50000},kind:{type:'string',enum:['note','question','hint','reference','attempt'],description:'显示与导出用途；hint/reference/attempt默认折叠且导出默认不含。只写已公开内容。'},placement:object({relativeTo:string('当前已有区域的准确标题。'),position:{type:'string',enum:['beside','below']}},['relativeTo','position']),interactive:object({provider:{type:'string',enum:['math']},preset:{type:'string',enum:['parabola']},scene:object({preset:{type:'string',enum:['parabola']},parameters:object({a:{type:'number'},h:{type:'number'},k:{type:'number'}}),observation:{type:'string',maxLength:1200}},['preset'])},['provider','preset','scene'])},['title','body']),write:true},
